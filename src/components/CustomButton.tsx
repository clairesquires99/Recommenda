...
const variantClasses: Record<ButtonVariant, { pressable: string; text: string }> = {
...
const sizeClasses: Record<ButtonSize, { pressable: string; text: string }> = {
  ...
  lg: { pressable: "min-h-[56px] px-6 gap-2",  text: "text-ds-body-lg font-bold" },
};
...
export const CustomButton: React.FC<ButtonProps> = ({
  ...
  return (
    <Pressable
      onPress={onPress}
      className={`flex-row items-center justify-center rounded-ds-pill border-2 w-full max-w-[400px] mx-auto ${v.pressable} ${s.pressable}${disabled ? " opacity-[0.45]" : ""}${className ? ` ${className}` : ""}`}
      disabled={disabled}
      {...props}
    >
      {Icon && <Icon />}
